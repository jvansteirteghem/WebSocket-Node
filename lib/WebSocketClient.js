...
WebSocketClient.prototype.connect = function(requestUrl, requestOptions, protocols, origin) {
    var self = this;
    ...
    requestOptions.path = this.url.pathname;
    if (this.url.search) {
        requestOptions.path += this.url.search
    }
    ...
    if (this.protocols.length > 0) {
        requestOptions.headers['Sec-WebSocket-Protocol'] = this.protocols.join(', ');
    }
    if (this.origin) {
        if (this.config.webSocketVersion === 13) {
            ...
            requestOptions.headers['Sec-WebSocket-Origin'] = this.origin;
        }
    }
